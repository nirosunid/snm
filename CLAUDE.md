...
### Production deploy
GitHub Actions workflow `.github/workflows/deploy-main.yml` does WireGuard install → SSH key setup → write `.env` from `PRODUCTION_ENV` secret → stream a gzipped source tarball over SSH. The deploy key's forced-command runs `scripts/receive-release.sh` (installed under a root-owned `bin/` on the server): it extracts the tarball into `releases/<ts>/`, atomically flips the `current` symlink, then runs `scripts/deploy.sh` from there, and prunes old releases (keeps last `KEEP`, default 5). Atomic releases rely on `name: smn` in `docker-compose.yml` so the Compose project (and image/layer cache) stays stable across timestamped release dirs. Server-side `deploy.sh`: rebuild → `docker compose up -d` → backup → migrate → image prune. Required GitHub secrets: `WG_PRIVATE_KEY`, `WG_SERVER_PUBLIC_KEY`, `WG_ENDPOINT`, `DEPLOY_SSH_KEY`, `DEPLOY_USER`, `PRODUCTION_ENV`.
